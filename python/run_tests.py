...
def get_class_rf(i, rf_classes):
    return int(rf_classes[i])
...
classifiers = ["Random_Forests", "Logistic_Regression", "kNN", "Decision_Tree", "SVM"]
...
rf = RandomForestClassifier().fit(train_rows, train_classes)
...
nbrs = NearestNeighbors(n_neighbors=5, algorithm='ball_tree',p=1).fit(train_rows, train_classes)
...
for IMAGES_FOLDER in settings.TEST_FOLDERS:
    ...
    for filename in os.listdir("\\".join([os.getcwd(), settings.TEST_SETS_FOLDER , IMAGES_FOLDER])):        
        ...
        print "Classifying through Random Forests"
        rf_classes = rf.predict(image_data_noclass)
        im_rf = im.copy()
        ...
        for i in range(len(valid_contours)):
            ...
            print "Painting with random forests"
            color_rf = settings.COLORS_RGB[get_class_rf(i, rf_classes)]
            cv2.drawContours(im_rf, [valid_contours[i]], -1, color_rf, -1)            
        ...
        cv2.imwrite("\\".join([settings.TEST_SETS_FOLDER, IMAGES_FOLDER, "Random_Forests", "contours - "+filename]), im_rf)
